...
PromptLabel 尽量把连续标注里的高频动作放到键盘上：`A` / `D` 切换图片，`1` - `9` 或 `Tab` 切换当前类别，`Q` / `Space` 开关 SAM，`R` 提交提示词。配合自动保存、状态栏反馈和右侧标注列表，常见流程可以少点鼠标、少打断地完成。
...
打开目录后，当前目录下的图片会作为“图片队列”显示在左侧：缩略图、文件名和图片总数都能直接看到。队列项带复选框，可勾选多张图片后一次提交提示词进行批量智能标注。连续标注时不用反复打开文件选择器，也更容易快速扫完整个目录。
...
主界面围绕“图片队列 - 画布 - 管理面板 - SAM 工作流”组织，适合连续切图、标注和校对。数据集处理和标注管理也保持同样的紧凑信息密度，方便在一个工具里完成整理、标注、复核和导出。

| 主工作台 | 数据集处理 |
| --- | --- |
| ![PromptLabel 主界面](assets/readme_main_ui.png) | ![PromptLabel 数据集处理](assets/readme_dataset_tool.png) |

| 类别与标注管理 | 快捷键速览 |
| --- | --- |
| ![PromptLabel 标注管理](assets/readme_annotation_tabs.png) | ![PromptLabel 快捷键速览](assets/shortcut.jpg) |

### 更少打扰的小优化

- 提示词下拉框滚动只切换内容，不会误提交 SAM 提示词。
- 左侧图片队列可勾选多张图片，提交提示词时按当前标签批量执行智能标注。
- 标签下拉框支持鼠标滚轮顺滑切换当前类别。
...
- 图片目录：缩略图队列、复选框多选、图片数量统计、上一张/下一张、批量提示词标注、右键删除图片及标注
...
- 左侧图片队列勾选多张图片后，点击提交 SAM 提示词会按当前标签逐张批量智能标注。
