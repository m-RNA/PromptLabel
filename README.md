...
数据集处理工具保留在独立窗口里，用于训练集划分和格式转换：

![PromptLabel 数据集处理](assets/readme_dataset_tool.png)

右侧标注管理按标注类型分组，标签页溢出时会显示滚动小三角：

![PromptLabel 标注类型标签页](assets/readme_annotation_tabs.png)
...
- 画布右键：打开/关闭 SAM、切换当前标签、新建标签；当鼠标位于已选标注上时，可批量修改选中标注类别。
